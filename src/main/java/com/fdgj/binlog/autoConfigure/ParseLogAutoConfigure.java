		...
		try {
			...
			while (open) {
				...
				try {
					...
					logger.debug("DefaultParseLog.parse() 处理失败, 回滚数据:batchId=" + batchId);
					e.printStackTrace();
					connector.rollback(batchId); // 处理失败, 回滚数据
				}
			}
			logger.debug("DefaultParseLog.parse()  normal stop and exit...");
		} finally {
			if (connector != null) {
				logger.debug("DefaultParseLog.parse() 释放链接");
	...
	protected void parseRowData(List<RowData> rowDatas, String database, String tableName, EventType eventType)
		...
		for (RowData rowData : rowDatas) {
			...
			if (eventType == CanalEntry.EventType.DELETE) {
				logger.debug("DefaultParseLog.parseRowData() 删除数据;database:" + database + ", tableName:" + tableName);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					logger.error(this.getClass().getName() + "启动Binlog失败");
					e.printStackTrace();
				}
			}
		}, "binlogThread").start();
